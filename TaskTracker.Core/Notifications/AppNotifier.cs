...
{
    public class AppNotifier : TaskTrackerDomainServiceBase, IAppNotifier
    {
        ...
        public async Task NewUserRegisteredAsync(User user)
        {
            var notificationData = new LocalizableMessageNotificationData(
                new LocalizableString(
                    ...
                    TaskTrackerConsts.LocalizationSourceName
                    )
                );
        ...
        public async Task NewTaskAssignedEventAsync(string message, string filterText, UserIdentifier targetUserId)
        {
            var notificationData = new MessageNotificationData(message);
            notificationData["filterText"] = filterText;

            await _notificationPublisher.PublishAsync(AppNotificationNames.NewTaskAssigned, notificationData, severity: NotificationSeverity.Success, userIds: new[] { targetUserId });
        }

        public async Task NewTaskClosedEventAsync(string message, string filterText, UserIdentifier targetUserId)
        {
            var notificationData = new MessageNotificationData(message);
            notificationData["filterText"] = filterText;
