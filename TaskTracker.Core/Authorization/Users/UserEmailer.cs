...
namespace TaskTracker.Authorization.Users
{
    ...
    /// </summary>
    public class UserEmailer : TaskTrackerServiceBase, IUserEmailer, ITransientDependency
    {
        ...
        public async Task SendPasswordResetLinkAsync(User user)
        {
            ...
            var link = _webUrlService.GetSiteRootAddress(tenancyName) + "Account/ResetPassword" +
                       "?userId=" + Uri.EscapeDataString(SimpleStringCipher.Instance.Encrypt(user.Id.ToString())) +
                       "&resetCode=" + Uri.EscapeDataString(user.PasswordResetCode) + "&tenantId=" + Uri.EscapeDataString(SimpleStringCipher.Instance.Encrypt(user.TenantId.ToString()));
