...
namespace TaskTracker.Notifications
{
    [AbpAuthorize]
    [DisableAuditing]
    public class NotificationAppService : TaskTrackerAppServiceBase, INotificationAppService
    {
        ...
        public async Task<GetNotificationsOutput> GetUserNotifications(GetUserNotificationsInput input)
        {
            ...
            var filteredNotifications = notifications.WhereIf(
                    !input.Filter.IsNullOrWhiteSpace(),
                    u =>
                        u.Notification.Data.Properties.Values.Contains(input.Filter)
                ).ToList();

            return new GetNotificationsOutput(totalCount, unreadCount, filteredNotifications);
        }
